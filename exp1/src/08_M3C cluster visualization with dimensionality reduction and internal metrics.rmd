...
rna.meta.path   = file.path(config$exp$parent, config$file_05$rna.meta.path)
...
```{r import}
# import metadata ----
rna_seq_metadata = readRDS(file.path(rna.meta.path, config$file_05$metadata_name))
...
saveRDS(entropy.pca, file = config$clustering$entropy.pca.out)
```
